...
export const updateSessionStatus = async (req: Request, res: Response) => {
  try {
    ...
    const session = await Session.findById(req.params.id) as ISession;

    if (!session) {
      return res.status(404).json({ message: 'Session not found' });
    }

    if (session.mentor.toString() !== (req.user as IUser)._id.toString() && 
        session.mentee.toString() !== (req.user as IUser)._id.toString()) {
...
export const submitFeedback = async (req: Request, res: Response) => {
  try {
    ...
    session.feedback = {
      rating,
      comment,
      givenBy: (req.user as IUser)._id,
    };
    await session.save();

    // Update user rating
    const targetUser = await User.findById(
      (req.user as IUser)._id.toString() === session.mentor.toString() 
        ? session.mentee 
        : session.mentor
    );
    ...
    const stats = await Session.aggregate([
      {
        $match: {
          $or: [
            { mentor: (req.user as IUser)._id },
            { mentee: (req.user as IUser)._id }
          ]
        }
      },
      {
